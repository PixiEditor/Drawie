...
{
     private readonly VulkanInteropContext _context;
    ...
    public unsafe VulkanContent(VulkanInteropContext context)
    {
        ...
        var device = _context.LogicalDevice.Device;
    ...
    public unsafe void DestroyTemporalObjects()
    {
        if (_isInit)
        {
            if (_renderPass.Handle != 0)
            {
                var api = _context.Api;
                var device = _context.LogicalDevice.Device;
                api.FreeDescriptorSets(_context.LogicalDevice.Device, _context.DescriptorPool, new[] { _descriptorSet });
