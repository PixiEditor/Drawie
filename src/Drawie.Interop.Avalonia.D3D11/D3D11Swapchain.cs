...
class D3D11Swapchain : SwapchainBase<D3D11SwapchainImage>
{
    ...
    {
        var image = new D3D11SwapchainImage(_device, size, Interop, Target);
        return image;
    }

    public IDisposable BeginDraw(PixelSize size, out D3D11SwapchainImage texture)
    {
        var rv = BeginDrawCore(size, out var img);
        texture = img;
        return rv;
    }
}
